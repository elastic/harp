...
func adjustMinVersion(options *Options, config *tls.Config) error {
	if options.MinVersion > 0 {
		if !isValidMinVersion(options.MinVersion) {
			return fmt.Errorf("tlsconfig: invalid minimum TLS version: %x", options.MinVersion)
		}
		if options.MinVersion < config.MinVersion {
			return fmt.Errorf("tlsconfig: requested minimum TLS version is too low. Should be at-least: %x", config.MinVersion)
		}
